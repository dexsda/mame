...
//				entry[menu_items] = field;
				...
				{
					case 0:	subtext.cpy("Off");	break;
					case 1:	subtext.cpy("On");		break;
					case 2:	subtext.cpy("Toggle");	break;
				}
	...
	for (i = 0; i < players; i++)
	{
		text.printf("P%d %s", i + 1, "Autofire Delay");
		subtext.printf("%d", machine().ioport().get_autofiredelay(i));

		/* append a menu item */
		item_append(text, subtext, MENU_FLAG_LEFT_ARROW | MENU_FLAG_RIGHT_ARROW, (void *)(intptr_t)(i + AUTOFIRE_ITEM_P1_DELAY));
	}
}
...
//	item_append(MENU_SEPARATOR_ITEM, NULL, 0, NULL);
	...
	for (port = machine().ioport().first_port(); port != NULL; port = port->next())
		for (field = port->first_field(); field != NULL; field = field->next())
		{
			...
			{
				const char colorbutton1 = is_neogeo ? '1' : '1';
				int n = 1;
				...
				for (i = 0; i < MAX_NORMAL_BUTTONS; i++, n <<= 1)
					if (machine().ioport().m_custom_button[player][type] & n)
					{
						if (subtext.len() > 0)
							subtext.cat(" +");
						subtext.catprintf(" %c", colorbutton1 + i);
					}
